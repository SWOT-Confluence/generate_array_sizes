...
# Deploy terraform
terraform init -reconfigure -backend-config="bucket=$S3_STATE" -backend-config="key=generate-array-size.tfstate" -backend-config="region=us-west-2" -backend-config="profile=$PROFILE"
